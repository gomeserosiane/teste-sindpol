...
export async function enviarPdfParaAssinafy(pdfBuffer, formulario) {
  const nome = limparTexto(formulario.dadosProponente?.nome) || "cadastro";
...
export async function criarAssinante(formulario) {
  ...
  const nome = limparTexto(proponente.nome);
  const email = limparTexto(proponente.email).toLowerCase();
  const whatsapp = normalizarTelefoneE164(proponente.telefone);
