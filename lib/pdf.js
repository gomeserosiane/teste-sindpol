...
function preencherProponentesAdicionais(page, proponentes = [], font) {
  const colunas = [
  ...
  proponentes.slice(0, 3).forEach((item, index) => {
...
function preencherValorTotal(page, formulario, fontBold) {
  ...
  const forma = texto(formulario.formaPagamento?.forma || "");
  if (forma) {
    page.drawText(`Forma de pagamento: ${forma}`, {
      x: 36,
      y: 202,
      size: 8,
      font: fontBold,
...
export async function gerarPdfPreenchido(formulario) {
  ...
  campoLinha(page, formulario.dadosProponente || {}, {
    nome: 134,
    nascimento: 154,
    sexo: 175,
    estadoCivil: 195,
  ...
  preencherProponentesAdicionais(page, formulario.proponentesAdicionais || [], font);
